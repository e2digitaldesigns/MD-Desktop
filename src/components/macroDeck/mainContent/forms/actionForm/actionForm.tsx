...
import MediaControlActionParser from "./parsers/mcActionParser";
...
const ActionForm: React.FC<{}> = () => {
  ...
  const handleActionChange = (
    ...
    type tSubActionObj = { [key: string]: string };

    const subActionObj: tSubActionObj = {
      obs: "obsLayerHide",
      md: "mdHome",
      mc: "mcAudioPause"
    };

    const subAction = subActionObj?.[action as keyof tSubActionObj] || "";
  ...
  const submit = () => {
    const newState = _cloneDeep(state);
    if (
      newState.action !== ApplicationActions.MC &&
